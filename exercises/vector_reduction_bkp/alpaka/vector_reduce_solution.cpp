...
struct ReduceKernel
{
        ...
        Idx const n) const -> void
    {
        ...
        auto const blockIdx = static_cast<uint32_t>(alpaka::getIdx<alpaka::Grid, alpaka::Blocks>(acc)[0]);
        auto const threadIdx = static_cast<uint32_t>(alpaka::getIdx<alpaka::Block, alpaka::Threads>(acc)[0]);
        auto const gridDim = static_cast<uint32_t>(alpaka::getWorkDiv<alpaka::Grid, alpaka::Blocks>(acc)[0]);

        // Get runtime block dimension (not compile-time TBlockSize!)
        auto const blockDim = static_cast<uint32_t>(alpaka::getWorkDiv<alpaka::Block, alpaka::Threads>(acc)[0]);

        // Calculate global thread index and grid stride using RUNTIME dimensions
        auto const globalIdx = blockIdx * blockDim + threadIdx;
        auto const gridStride = gridDim * blockDim;

        // Grid-stride reduction: accumulate multiple elements per thread
        T sum = 0;
        for(Idx idx = globalIdx; idx < n; idx += gridStride)
        {
            sum += input[idx];
        }
        ...
        // Simple tree-based reduction in shared memory
        // Start with stride = blockDim/2 and halve each iteration
        for(uint32_t stride = blockDim / 2; stride > 0; stride >>= 1)
        {
            if(threadIdx < stride)
            {
        ...
        // Thread 0 writes the block's final result to global memory
        if(threadIdx == 0)
        {
